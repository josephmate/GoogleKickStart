...
fn solve(
    num_houses: i32,
    budget: i32,
    mut house_prices: Vec<i32>
) -> i32 {
    house_prices.sort();
    let mut used_budget = 0;
    let mut houses_bought = 0;
    for house_idx in 0..num_houses {
        let house_price = house_prices[house_idx as usize];
        if house_price + used_budget > budget {
            return houses_bought;
        }
        houses_bought += 1;
        used_budget += house_price;
    }
    return houses_bought;
}
...
fn handle_test_case() {
    match parse_pair_int_line() {
        Ok((num_houses, budget)) => {
            match parse_int_vector_line() {
                Ok(house_prices) => {
                    let result = solve(num_houses, budget, house_prices);
                    println!("{}", result);
                },
